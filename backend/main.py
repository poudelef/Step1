...
class VoiceInterviewResponse(BaseModel):
    persona_response: str
    persona_audio_url: str = None  # URL to synthesized speech response
    transcribed_user_message: str = None
    suggested_questions: List[str]
...
async def call_groq(prompt: str, system_prompt: str = "You are a helpful AI assistant.") -> str:
    ...
    payload = {
        "model": "mixtral-8x7b-chat",  # Updated model name
        "messages": [
...
async def call_claude(prompt: str, system_prompt: str = "You are a helpful AI assistant.") -> str:
    ...
    async with httpx.AsyncClient(timeout=60) as client:
        ...
        if resp.status_code != 200:
            raise HTTPException(
                status.HTTP_502_BAD_GATEWAY,
                detail=f"Claude error: {resp.text}",
            )
...
{{
    ...
    try:
        raw_json = await call_gpt4o(prompt, system_prompt)
...
{{
    ...
    "value_propositions": ["value prop 1", "value prop 2", "value prop 3"]
        ...
        structured_response = await call_gpt4o(structure_prompt)
...
async def debug_groq():
    ...
    try:
        response = await call_gpt4o("Say hello!", "You are a helpful assistant.")
        return {"success": True, "response": response[:100] + "..." if len(response) > 100 else response}
    except Exception as e:
        return {"success": False, "error": str(e), "error_type": type(e).__name__}
...
async def test_all_apis():
    ...
    # Test OpenAI
    try:
        openai_result = await call_gpt4o("Say 'OpenAI working!'", "You are a test assistant.")
